...
const getServiceIcon = (iconName: string | null) => {
    ...
    // @ts-ignore - Dynamic access to Lucide library
...
const Booking = () => {
    ...
    const renderStep1 = () => {
        ...
        // This service holds the "Time Slots" (variants) and the "Generic Description"
        const baseService = services.find(s => !s.is_visible || s.slug.includes('video') || s.slug.includes('consulta-judicial')) || services[0];

        // 2. Identify the TARGET Service (The topic: Divorce, Contracts, etc.)
        // If no pre-selection, the Target IS the Base service.
        const targetService = preselectedServiceId
            ? services.find(s => s.id === preselectedServiceId) || baseService
            : baseService;

        // 3. Prepare Data
        // Always use variants from BASE (Time slots)
        const timeVariants = (baseService?.variants || [])
            .filter(v => v.is_active)
            .sort((a: any, b: any) => a.order_index - b.order_index);

        // Logic to show the Gold Subtitle only if we are in a specific topic
            ...
            <div className="space-y-8 animate-in slide-in-from-right duration-300">
                ...
                {loadingServices ? (
                    ...
                    <div className="flex flex-col gap-8">
                        {/* Render Single Card Mixing Data */}
                            ...
                            <div className="p-5 md:p-8 bg-white relative z-10">
                                <div className="flex flex-col md:flex-row md:items-start gap-6">
                                    ...
                                    <div className="p-4 bg-navy-50 rounded-2xl text-navy-900 group-hover:bg-gold-500 group-hover:text-white transition-all duration-300 shadow-sm flex-shrink-0">
                                        {getServiceIcon(baseService.icon_name || 'Briefcase')}
                                    </div>

                                    <div className="flex-1">
                                        {/* TITLE: Fixed */}
                                        <h4 className="font-bold text-2xl text-navy-900 mb-2">{baseService.name}</h4>

                                        {/* DESCRIPTION: Comes from BASE (Generic info) */}
                                        {!isSpecificTopic && (
                                            <p className="text-gray-500 text-base leading-relaxed max-w-4xl mb-2">
                                                {baseService?.description}
                                            </p>
                                        )}

                                        {/* SUBTITLE: Comes from TARGET (Topic Name) - Only if specific */}
                                        {isSpecificTopic && (
                                            <p className="text-gold-500 font-bold text-sm uppercase tracking-wider animate-in fade-in slide-in-from-left-1">
                                                {targetService?.name}
                                            </p>
                                        )}
                                    </div>
                            ...
                            <div className="p-5 md:p-8 bg-gray-50/80 border-t border-gray-100 flex-1">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                                    {timeVariants.length === 0 && (
                                        <div className="text-gray-400 italic col-span-full text-center py-4">No hay horarios disponibles</div>
                                    )}
                                    {/* VARIANTS: Come from BASE (Time slots) */}
